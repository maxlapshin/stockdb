...
full_md_test() ->
  ...
  ?assertEqual({Timestamp, BidAsk, byte_size(Bin)}, stockdb_format:decode_full_md(<<Bin/binary, Tail/bitstring>>, 2)),
  ?assertEqual({full_md, {Timestamp, BidAsk}, byte_size(Bin)}, stockdb_format:decode_packet(<<Bin/binary, Tail/bitstring>>, 2)).
...
delta_md_test() ->
  ...
  ?assertEqual({DTimestamp, DBidAsk, byte_size(Bin)}, stockdb_format:decode_delta_md(<<Bin/bitstring, Tail/bitstring>>, 2)),
  ?assertEqual({delta_md, {DTimestamp, DBidAsk}, byte_size(Bin)}, stockdb_format:decode_packet(<<Bin/bitstring, Tail/bitstring>>, 2)).
...
trade_test() ->
  ...
  ?assertEqual({trade, {Timestamp, Price, Volume}, byte_size(Bin)}, stockdb_format:decode_packet(<<Bin/binary, Tail/binary>>, 42)).
